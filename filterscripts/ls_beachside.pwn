...
public OnFilterScriptInit()
{
    ...
    print("  |--  LS BeachSide Building Elevator loaded");
...
public OnFilterScriptExit()
{
	...
	// Display information in the Server Console
    print("  |--  LS BeachSide Building Elevator unloaded.");
    ...
    for (new i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (IsPlayerConnected(i) && !IsPlayerNPC(i))
        {
	    	...
	    	SetPVarInt(i, "ElevatorSound", 0);
	    	KillTimer(evtimerexp[i]);
	    	KillTimer(evtimer[i]);
        }
    }
    // Exit here
...
public OnPlayerConnect(playerid)
{
	...
	evtimer[playerid] = SetTimerEx("EvMusicCheck", 1000, true, "i", playerid);
	SetPVarInt(playerid, "ElevatorSound", 0);
...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Apasa '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' pentru a folosi liftul", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, z - 0.4, 4.0, 0, 1);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_ID)
    {
        ...
        if(FloorRequestedBy[listitem] != INVALID_PLAYER_ID || IsFloorInQueue(listitem))
            GameTextForPlayer(playerid, "~r~Etajul este deja pe lista de asteptare", 3500, 4);
		else if(DidPlayerRequestElevator(playerid))
		    GameTextForPlayer(playerid, "~r~Deja ai chemat liftul", 3500, 4);
		else
				...
				if (i <= 12)
				{
				    ...
				    if (ElevatorState != ELEVATOR_STATE_MOVING)
				    {
				        ...
				        if (ElevatorFloor == i + 1)
				        {
							...
							GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~y~~h~Liftul~n~~y~~h~este deja la acelasi etaj cu tine.~n~~w~Intra in el si apasa~w~ '~k~~CONVERSATION_YES~'", 3500, 3);

							// Display chat text message to the player
	                        SendClientMessage(playerid, COLOR_MESSAGE_YELLOW, "* Liftul este deja la acelasi etaj cu tine. Intra in el si apasa '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}'");
					...
					// Display gametext message to the player
					GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~g~~h~Liftul~n~~g~~h~a fost chemat...~n~~w~Te rog asteapta", 3000, 3);
					...
					if (ElevatorState == ELEVATOR_STATE_MOVING)
					{
						...
						format(strTempString, sizeof(strTempString), "* Liftul a fost chemat, momentan este in drum spre %s.", FloorNames[ElevatorFloor]);
					}
					else
					{
					    ...
					    {
					    	// Format chat text message
							format(strTempString, sizeof(strTempString), "* Liftul a fost chemat, momentan este la %s.", FloorNames[ElevatorFloor]);
						}
						else
						{
					    	// Format chat text message
...
stock Elevator_Initialize()
{
	...
	Label_Elevator = Create3DTextLabel("{CCCCCC}Apasa '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' pentru a folosi liftul.", 0xCCCCCCAA, X_ELEVATOR_POS + 1.6, Y_ELEVATOR_POS - 1.85, GROUND_Z_COORD - 0.4, 4.0, 0, 1);
	...
	for (new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Apasa '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' pentru a folosi liftul.", FloorNames[i]);
